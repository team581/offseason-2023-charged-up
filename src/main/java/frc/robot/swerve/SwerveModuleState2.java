...
public class SwerveModuleState2 extends SwerveModuleState {
  ...
  public static SwerveModuleState2 optimize(
    ...
    SwerveModuleState2 optimized =
        new SwerveModuleState2(
            SwerveModuleState.optimize(desiredState, currentAngle), desiredState.omegaRadPerSecond);
    if (desiredState.angle.equals(currentAngle)
        || desiredState.angle.equals(optimized.angle.rotateBy(Rotation2d.fromDegrees(90)))
        || moduleSteerFeedForwardClosedLoop == 0) {
  ...
  public SwerveModuleState2 invert() {
    ...
    return new SwerveModuleState2(
        -speedMetersPerSecond, angle.rotateBy(Rotation2d.fromDegrees(180)), -omegaRadPerSecond);
  }
